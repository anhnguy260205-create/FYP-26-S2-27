...
function SignupsBarChart({ series, loading }) {
  ...
  const labelIndices = series.length > 1
  ...
  return (
    <div className="relative">
      <div className="flex">
        {/* Fixed y-axis — stays put while the plot scrolls horizontally */}
        <svg
          width={Y_AXIS_WIDTH}
          height={CHART_HEIGHT}
        ...
        <div className="overflow-x-auto flex-1">
          <svg
            ...
            height={CHART_HEIGHT}
            role="img"
            aria-label="New user signups per day"
          ...
          >
            <defs>
              ...
              <linearGradient id="signupBarHover" x1="0" y1="0" x2="0" y2="1">
            ...
            {series.map((point, i) => {
              ...
              return (
                <g key={point.date}>
                  <path
                    d={roundedTopBarPath(x, PADDING_TOP + plotHeight - barHeight, barWidth, barHeight, 5)}
                    fill={isHovered ? "url(#signupBarHover)" : "url(#signupBar)"}
                    style={{
                      transformBox: "fill-box",
                      transformOrigin: "bottom",
                      transform: grown ? "scaleY(1)" : "scaleY(0)",
                      transition: `transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1) ${i * 12}ms`,
                    }}
                  />
                  <rect
                    x={i * slotWidth}
                    ...
                    tabIndex={0}
                    role="button"
                    aria-label={`${formatFullDate(point.date)}: ${point.count} new user${point.count === 1 ? "" : "s"}`}
                    onMouseEnter={(e) => {
                      setHoverIndex(i);
      ...
      {hovered && (
        ...
        >
          <div className="flex items-center gap-1.5 font-bold text-sm">
            ...
            {hovered.count} new user{hovered.count === 1 ? "" : "s"}
          </div>
