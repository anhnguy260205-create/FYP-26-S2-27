...
def _is_privileged(current_user: Optional[dict]) -> bool:
    """Check whether the user can access gated forum content."""
    if not current_user:
        return False
    return (
...
# Request data used when creating a post
...
# Fields that can be changed when editing a post
...
# Request data used when replying to a post
class ReplyRequest(BaseModel):
    content: str


# Request data used when editing a reply
...
def get_post(
    ...
    return ForumController().get_post(
        post_id, user_id,
        viewer_is_privileged=_is_privileged(current_user)
    )


# Request data used for stock commentary
...
# Let verified experts post stock commentary
...
def create_stock_comment(
    ...
    if current_user.get("is_expert") is not True:
        raise HTTPException(
            status_code=403,
            detail="Only verified experts can post stock commentary."
        )
    return ForumController().create_stock_comment(
        current_user["user_id"], symbol, data.content
    )
...
def delete_post(post_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return ForumController().delete_post(
        post_id, current_user["user_id"], is_admin=is_admin
    )
...
def delete_reply(
    ...
    is_admin = current_user.get("role") == "admin"
    return ForumController().delete_reply(
        post_id, reply_id, current_user["user_id"], is_admin=is_admin
    )


# Request data used when reporting content
...
def flag_post(
    post_id: str,
    data: FlagPostRequest,
    current_user: dict = Depends(get_current_user)
):
    return ForumController().flag_post(
        post_id, current_user["user_id"], data.reason
    )
...
def flag_reply(
    ...
    return ForumController().flag_reply(
        reply_id, current_user["user_id"], data.reason
    )


# Get the user's content removal notice
...
# Request data used to acknowledge a removal notice
class AcknowledgeRemovalRequest(BaseModel):
    removal_id: str


# Mark a removal notice as acknowledged
@router.post("/removal-notice/acknowledge")
def acknowledge_removal(
    data: AcknowledgeRemovalRequest,
    current_user: dict = Depends(get_current_user)
):
    return ForumController().acknowledge_removal(
        data.removal_id, current_user["user_id"]
    )
...
# Clear flags from a post
@router.delete("/admin/{post_id}/flags")
def admin_clear_post_flags(
    post_id: str,
...
# Clear flags from a reply
@router.delete("/admin/replies/{reply_id}/flags")
def admin_clear_reply_flags(
    reply_id: str,
    current_user: dict = Depends(require_admin)
):
...
# Request data used when removing content as an admin
class AdminDeletePostRequest(BaseModel):
    reason: str = "Violated community guidelines"


# Remove a post as an admin
@router.delete("/admin/{post_id}")
def admin_delete_post(
    post_id: str,
    data: AdminDeletePostRequest,
    current_user: dict = Depends(require_admin)
):
    return ForumController().admin_delete_post(
        post_id,
...
def admin_delete_reply(
    ...
    return ForumController().admin_delete_reply(
        post_id, reply_id, reason=data.reason
    )