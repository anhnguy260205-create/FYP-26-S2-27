...
from app.entity.models.landingcontent import ContentManagement
...
def get_landing_content():
    items = ContentManagement.get_all()
...
def update_content(content_id: str, data: UpdateContentRequest):
    success = ContentManagement.update(content_id, data.title, data.description)
    if not success:
